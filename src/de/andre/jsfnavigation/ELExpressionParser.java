...
public final class ELExpressionParser {
    ...
    public static ELExpression find(IDocument document, int cursorOffset) {
        try {
            ...
            String rawBody = text.substring(start + 2, end);
            int leadingWhitespace = 0;
            while (leadingWhitespace < rawBody.length()
                    && Character.isWhitespace(rawBody.charAt(leadingWhitespace))) {
                leadingWhitespace++;
            }

            String body = rawBody.substring(leadingWhitespace).trim();
            if (body.isEmpty()) {
                return null;
            }

            List<String> parts = splitSimplePropertyChain(body);
            if (parts.isEmpty()) {
                return null;
            }

            return new ELExpression(body, parts, start + leadingWhitespace);
    ...
    private static List<String> splitSimplePropertyChain(String body) {
