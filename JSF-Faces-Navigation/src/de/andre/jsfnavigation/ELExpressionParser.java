    ...
    public static ELExpression find(IDocument document, int cursorOffset) {
        try {
            ...
            while (chainMatcher.find()) {
                ...
                List<Boolean> methodInvocations = new ArrayList<Boolean>();
                ...
                while (partMatcher.find()) {
                    ...
                    /*
                     * The chain regex deliberately ends before method
                     * parentheses.  Inspect the original EL body as well so
                     * `bean.store()` can be resolved as a method rather than
                     * as a property called `store`.
                     */
                    int absoluteAfterPart =
                            offset + part.length();

                    while (absoluteAfterPart < end
